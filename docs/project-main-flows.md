...
4. **Receiving side** — mobile subscribes to the room's events topic via `subscribeRoom()` (`packages/mqtt-client/src/client.ts:162`, invoked from `useRoom.ts` on room entry). The event-handling chain on mobile has four hops:
   1. **Raw MQTT entry** — `handleMessage()` (`client.ts:45`), wired via `connection.on('message', handleMessage)` (`client.ts:142`). It dispatches by topic: presence topics go to presence listeners; topics ending in `/events` are `JSON.parse`d into a `ServerEvent` and broadcast to all `eventListeners` (`client.ts:69`).
   2. **Listener registration** — `apps/mobile/src/contexts/MqttContext.tsx:59`: on connect, the store handler is attached with `next.onEvent(handleServerEvent)` (`onEvent` itself just adds to the `eventListeners` set, `client.ts:200`).
   3. **Event dispatch** — `handleServerEvent()` (`apps/mobile/src/stores/roomStore.ts:86`): a `switch` on `event.type` that currently only handles `message.delivered` by calling `appendMessage(event.message)`. Agent replies and human messages take exactly the same path here — no distinction is made.
   4. **UI state** — `appendMessage()` (`roomStore.ts:74`): a Zustand `set()` that dedupes by `message.id`, appends to `messages` (drives the chat screen) and updates `lastMessages[roomId]` (drives the room-list preview).
