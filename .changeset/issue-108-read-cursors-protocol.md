...
- `@logact-pub/opc-protocol`：
  - `MQTT_TOPICS` 新增 `reads(roomId)`（`opc/rooms/{roomId}/reads`）与 `readsFilter`（`opc/rooms/+/reads`），并新增 `parseReadsTopic`；`parseRoomTopic` 支持 `reads` 方向。
  - 新增 `RoomReadsPayloadSchema`（`{ from, lastReadAt }`，lastReadAt 为 ISO 8601 消息时间戳）及导出类型 `RoomReadsPayload`。
